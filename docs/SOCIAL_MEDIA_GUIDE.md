...
**Composition variety, added 2026-08-28 per direct feedback ("nudna templatka, każdy post wygląda tak samo"):** the default so far has been small corner icon + huge left-aligned headline, on every single post - that's a layout choice, not a hard rule, and it was starting to look identical post to post. When a topic supports it, let the accent graphic itself become the dominant visual instead of a small corner decoration - see `assets/x_post_visual_builder_blocks.svg` (the block diagram fills the center of the frame, headline is shorter and wraps around it) and `assets/ig_carousel_visual_builder_blocks/` (the diagram literally builds one block per slide across slides 2-4, instead of 8 isolated text cards). Still keep: the palette, the wordmark position/size, typography, and - critically - slide 1 (hook) and the final CTA slide close to the proven text-forward format, since those two carry most of a carousel's outcome per the research below; experiment with composition on the slides in between, not those two anchors.
...
1. **Hook-first sentence.** The opening line must be a complete, compelling thought on its own — someone deciding whether to keep reading sees only that first line/paragraph in a crowded feed.
   - **Never name AlgoPuzzle in that opening sentence**, added 2026-08-28 per direct feedback. Leading with the brand name reads as an ad; leading with the reader's own problem or the fact itself reads as content — this matters more on a zero-recognition new account, where a stranger doesn't yet have any reason to care what "AlgoPuzzle" is. The brand still shows up - in the image's wordmark and in the link/bio - it just doesn't need to be the first thing said. In practice most posts already did this by default (e.g. "You can't watch a chart forever. An Expert Advisor can." / "'Take profit' and 'stop loss' sound like two more things to memorize."); the one exception that prompted this rule opened with "Every strategy **on AlgoPuzzle** reduces to..." - rewritten to "Every trading strategy, no matter how complex it sounds, reduces to..." Applies to the X main post's opening line specifically; the same instinct is worth applying to an Instagram carousel's slide 1 headline too, though that slide already tends not to need the brand name given the existing hook formula (bold headline + pattern interrupt + curiosity trigger) in the Instagram rules below.
...
4. **No hashtags at all, as of 2026-08-25 - an active, ongoing experiment, not a permanent reversal of the old "1-2 niche hashtags" rule.** Real X Analytics data on 6 posts (all of which *did* use 1-2 "niche" tags like `#AlgoTrading`/`#TradingStrategy`) showed a reply-to-like ratio that's a strong bot signature - e.g. the Aug 21 post had **more replies (12) than likes (10)**, which essentially never happens with a real audience (a like costs one tap, a reply costs effort - replies should always trail likes by a wide margin). The working theory: even "niche" algo-trading hashtags are still inside the finance/forex bot-scanning net. User directed testing several posts with zero hashtags and comparing the same reply-to-like ratio before concluding anything - see if it actually changes. **Until told otherwise, ship new X posts with no hashtags** - don't revert to the old rule from memory without being told the experiment concluded.
