...
express it in a given language. See HANDOFF.md for the story of why this
...
def _mql5_action_block(action: ActionIR, index: int,
    ...
    if action.direction == "BUY":
        open_call = f"trade.Buy(lotToTrade, {symbol_var}, 0, 0, 0, \"Scratch for Traders\")"
        ...
        open_call = f"trade.Sell(lotToTrade, {symbol_var}, 0, 0, 0, \"Scratch for Traders\")"
...
def render_mql5(ir: StrategyIR) -> str:
    ...
    for rule in ir.rules:
        ...
        init_parts.append(
            ...
            f"{rule_init}"
            f'      Print("Scratch for Traders EA - Rule {i + 1} initialized on ", {symbol_var}, " (magic ", {magic_var}, ")");\n'
...
"""
    ...
    return f"""SCRATCH FOR TRADERS - Setup Guide
===================================
...
initialization line PER RULE, e.g.:
    Scratch for Traders EA - Rule 1 initialized on {ir.rules[0].asset} (magic {ir.rules[0].magic})
...
Generated by Scratch for Traders (MVP).
"""
...
# HANDOFF.md. Written from the documented, stable parts of the cAlgo API;
...
# syntax (see HANDOFF.md); <<TOKEN>> placeholders can't collide with C#'s
# own `{`/`}` at all, so that whole class of mistake isn't possible here.
_CSHARP_TEMPLATE = """// -----------------------------------------------------------------
// ScratchForTradersBot.cs
// Auto-generated by "Scratch for Traders" (No-Code Builder)
// Generated: <<GENERATED_AT>>
//
// WARNING: Always test on a Demo account before going live.
...
namespace cAlgo.Robots
{
    ...
    public class ScratchForTradersBot : Robot
    {
        // Per-rule strategy configuration (from the visual block builder) -
...
def render_csharp(ir: StrategyIR) -> str:
    ...
    for rule in ir.rules:
        ...
        label = f"ScratchForTraders_Rule{i}"
        ...
        init_parts.append(
            ...
            f"{rule_init}"
            f'                Print("Scratch for Traders cBot - Rule {i + 1} initialized on {{0}}", {symbol_name_var});\n'
            f"            }}\n"
        )
...
def generate_readme_csharp(ir: StrategyIR) -> str:
    rule_summaries = "\n\n".join(
        f"Rule {r.rule_index + 1} (label ScratchForTraders_Rule{r.rule_index})\n"
        f"- Asset:      {r.asset}\n"
        f"- Timeframe:  {r.timeframe}\n"
        f"- Actions:    {_describe_actions(r)}\n"
        f"- Positions:  {_describe_positions(r)}"
...
"""
    ...
    return f"""SCRATCH FOR TRADERS - cTrader Setup Guide
=============================================

Your strategy has been compiled into a cTrader cBot
(ScratchForTradersBot.cs). Follow the steps below to install and run it.
...
the contents of "ScratchForTradersBot.cs" from this zip.
...
choose "ScratchForTradersBot" from the list, then click "Add to chart". It
...
one initialization line PER RULE, e.g.:
    Scratch for Traders cBot - Rule 1 initialized on {ir.rules[0].asset}
...
# deliberately removed it - see render_mql5()'s history in HANDOFF.md item
...
def _mql4_action_block(action: ActionIR, index: int, timeframe: str, symbol_var: str, magic_var: str) -> str:
    ...
    return (
        ...
        f"OrderSlippagePoints, 0, 0, \"Scratch for Traders\", {magic_var}, 0, clrNONE);\n"
        f"         if(ticket < 0)\n"
        f"           {{\n"
...
# hit once in the original MQL5 renderer (see HANDOFF.md item 1).
_MQL4_TEMPLATE = """//+------------------------------------------------------------------+
//|                                              strategy.mq4         |
//|         Auto-generated by "Scratch for Traders" (No-Code Builder) |
//|         Generated: <<GENERATED_AT>>                                |
...
"""
    ...
    return f"""SCRATCH FOR TRADERS - MT4 Setup Guide
=========================================
...
async def generate_expert_advisor(config: WorkspaceConfig, device_id: str = Depends(get_device_id)):
    ...
    filename = f"scratch_for_traders_{_asset_slug_for_filename(config)}.zip"
...
async def generate_cbot(config: WorkspaceConfig, device_id: str = Depends(get_device_id)):
    ...
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("ScratchForTradersBot.cs", csharp_code)
        zf.writestr("README.txt", readme_text)
    buffer.seek(0)

    filename = f"scratch_for_traders_ctrader_{_asset_slug_for_filename(config)}.zip"
...
async def generate_expert_advisor_mt4(config: WorkspaceConfig, device_id: str = Depends(get_device_id)):
    ...
    filename = f"scratch_for_traders_mt4_{_asset_slug_for_filename(config)}.zip"
