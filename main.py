...
(strategy.mq5). Follow the steps below to install and run it - or just
watch "Installation_Tutorial.mp4" in this zip, which walks through the
same steps on screen.
...
# --------------------------------------------------------------------------
# MT5 install walkthrough video - bundled into every MT5 export zip so a
# trader has a visual alongside README.txt's numbered steps. Read from disk
# once and cached in-process - the file is ~40MB, so re-reading it per
# request would be wasteful (and multiplies badly under concurrent
# downloads).
# --------------------------------------------------------------------------
MT5_TUTORIAL_VIDEO_PATH = Path(__file__).parent / "assets" / "tutorials" / "mt5_installation_tutorial.mp4"
MT5_TUTORIAL_VIDEO_ZIP_NAME = "Installation_Tutorial.mp4"


@lru_cache(maxsize=1)
def _mt5_tutorial_video_bytes() -> Optional[bytes]:
    """Returns the tutorial video's bytes, or None if the asset is missing
    (e.g. a dev checkout that hasn't pulled the large binary) - a missing
    video should never break an MT5 export, just ship without it."""
    try:
        return MT5_TUTORIAL_VIDEO_PATH.read_bytes()
    except OSError:
        return None


def _add_mt5_tutorial_video(zf: zipfile.ZipFile) -> None:
    video_bytes = _mt5_tutorial_video_bytes()
    if video_bytes is None:
        return
    # STORED, not DEFLATED: the .mp4 is already compressed, so re-deflating
    # it would just burn CPU on every export for no size benefit.
    zf.writestr(MT5_TUTORIAL_VIDEO_ZIP_NAME, video_bytes, compress_type=zipfile.ZIP_STORED)
